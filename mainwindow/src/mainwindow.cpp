...
void MainWindow::setupUi() {
  ...
  connect(save_map_btn, &QToolButton::clicked, [this]() {
    ...
    if (!fileName.isEmpty()) {
      ...
      LOG_INFO("Save the map path selected by the user :" << fileName.toStdString());
      display_manager_->SaveMap(fileName.toStdString());
    } else {
      // 用户取消了选择
      LOG_INFO("Cancel save map");
    }
  });
  connect(open_map_btn, &QToolButton::clicked, [this]() {
    ...
    if (!fileName.isEmpty()) {
      // 用户选择了文件夹，可以在这里进行相应的操作
      LOG_INFO("Open the map path selected by the user:" << fileName.toStdString());
